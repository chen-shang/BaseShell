...
function new_function() {
  _NotBlank "$1" "source function name can not be null" && _NotBlank "$2" "target function name can not be null"
...
function ssh_checkLogin() {
  _NotBlank "$1" "ip can not be null" && _NotBlank "$2" "port can not be null" && _NotBlank "$3" "user can not bull" && _NotBlank "$4" "pass can not bull"
  local ip=$1
  local port=$2
  local user=$3
  local pass=$4
...
new_lock() {
  _NotBlank "$1" "lock fd can not be null"
  local fd=$1
  new_fifo "${fd}"
  echo 0 >&"${fd}"
}
...
function lock_run() {
  _NotBlank "$1" "lock fd can not be null" && _NotBlank "$2" "function can not be null"
  local fd=$1
  shift
  local task=$*
  lock_tryLock "${fd}"
...
isExist() {
  _NotBlank "$1" "fd can not be null"
  local fd=$1
  {
    flock 3
    [[ -z "${fd}" ]] && {
...
new_threadPool() {
  _NotBlank "$1" "core size can not be null" && _Natural "$1" && _Min "0" "$1"
  local coreSize=$1
  local fd=$(new_fd)
  new_fifo "${fd}"
...
threadPool_submit() {
  _NotBlank "$1" "thread pool can not be null"
  local fd=$1
  shift
  local task=$*
